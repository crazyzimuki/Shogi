...
public class Leopard : PieceDATA
{
    private int bounds;
    ...
    public List<(int, int)> LeopardMove()
    {
    ...
    public List<(int, int)> FalconMove()
    {
            List<(int, int)> moves = new List<(int, int)>();
            int newRow = row;
            int newCol = col;
        ...
        { 1, -1 }, { 1, 0 }, { 1, 1 }  // Backward moves
            ...
            for (int i = 0; i < 7; i++) // For each movement direction
            {
                // Reset values
                newRow = row;
                newCol = col;
