...
public class Copper : PieceDATA
{
    private int bounds;

    public Copper(bool promoted)
    {
    ...
    public override List<(int, int)> GetLegalMoves()
    {
        ...
        if (doublepromoted)
            ...
            return CrabMove();
        else return CopperGeneralMove();
    }

    public List<(int, int)> CopperGeneralMove()
    {
        List<(int, int)> moves = new List<(int, int)>();

        // Define possible move offsets for a Copper General (relative to its position)
        int[,] moveOffsets = new int[,]
        {
        { -color, -1 }, { -color, 0 }, { -color, 1 }, // Forward moves 
        {  color,  1 }  // Backward moves
        };
    ...
    public List<(int, int)> CrabMove()
    {
        ...
        // Define infinite range move directions for a Crab (relative to its position)
        int[,] moveOffsets = new int[,]
        {
        ...
        for (int i = 0; i < 2; i++) // For each movement direction
        {
            ...
            while (newRow >= 0 && newRow < bounds && newCol >= 0 && newCol < bounds) // Keep going until out of bounds
            {
                ...
                if (newRow >= 0 && newRow < bounds && newCol >= 0 && newCol < bounds) // If still in bounds
                {
                    ...
                    {
                        //Debug.Log($"THE CRAB MOVE: Row == " + newRow + " Col == " + newCol + " CAPTURES AN ENEMY PIECE. ADDED TO LIST!");
                        moves.Add((newRow, newCol));
                        break;
                    }
                    ...
                    {
                        //Debug.Log($"THE CRAB MOVE: Row == " + newRow + " Col == " + newCol + " CAPTURES FRIENDLY PIECE. REJECTED!");
                        break;
                    }

                    else moves.Add((newRow, newCol));
                    //Debug.Log($"THE CRAB MOVE: Row == " + newRow + " Col == " + newCol + " ADDED TO LIST!");
                }
            }
        }

        // Define range 1 move directions for a Crab (relative to its position)
        moveOffsets = new int[,]
        {
        { 1, 0 },          // Forward move
        { -1, 0 },         // Backward move
